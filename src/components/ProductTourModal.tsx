...
type TourAudience = "companion" | "manager";

/**
 * Shell compartilhado do guia rápido (companion / RH).
 * Abre quando a API indica needsTour para o audience pedido.
 */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (result.status?.needsTour && result.status.audience === audience) {
          setOpen(true);
        }
      } catch {
        // silencioso
