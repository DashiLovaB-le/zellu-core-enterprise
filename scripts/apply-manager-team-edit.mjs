...
const name = "013_manager_team_edit";
const sql = readFileSync(resolve(root, "supabase/migrations/013_manager_team_edit.sql"), "utf8");
...
if (names.has(name)) {
  console.log(`Skip ${name} (já aplicada) — reaplicando DDL idempotente via query...`);
  await query(sql);
} else {
...
const fns = await query(`
  ...
  WHERE n.nspname = 'public'
    AND proname IN ('assign_team_member', 'list_company_directory')
  ORDER BY proname;
`);
...
const policy = await query(`
  ...
  WHERE polrelid = 'public.teams'::regclass
    AND polname = 'teams_manager_update';
`);
console.log("Policy teams_manager_update:", policy.length ? "ok" : "missing");
